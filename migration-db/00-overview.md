...
9. **Vérification de confiance** : `verification_status` gère trois états principaux — `unofficial` (données **non officielles**, accessibles mais non vérifiées par A.N.A.N.A.S.) et `verified` (données approuvées = **confiance officielle du site**). Un état `rejected` existe pour les données rejetées. Les nouveaux items ont par défaut `unofficial`.
