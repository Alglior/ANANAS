...
def admin_replication():
    return render_template(
        "admin/replication.html",
        title="Administration — Réplication",
        meta_description="Répliquer des catalogues depuis une instance distante",
    )
        ...
        while page <= total_pages:
            ...
            for item_data in items:
                ...
                for tag_name in item_data.get("tags", []):
                    tag = ItemTag(item_id=new_item.id, tag=sanitize_html(tag_name))
                    db.session.add(tag)

                for gallery_entry in item_data.get("gallery", []):
                    media_type = gallery_entry.get("type")
                    src = gallery_entry.get("src")
                    label = gallery_entry.get("label")
                    data_json = gallery_entry.get("data")
                    if media_type:
                        gal = ItemGallery(
                            item_id=new_item.id,
                            media_type=media_type,
                            src=src,
                            label=sanitize_html(label) if label else None,
                            data_json=data_json if isinstance(data_json, dict) else None,
                        )
                        db.session.add(gal)

                for vl in item_data.get("visualization_links", []):
                    viz = VisualizationLink(
                        parent_item_id=new_item.id,
                        name=sanitize_html(vl.get("name", "")),
                        ...
                        display_order=vl.get("display_order", 0),
                        is_active=vl.get("is_active", True),
                    )
                    db.session.add(viz)
                ...
                for dc in item_data.get("data_chunks", []):
                    chunk = DataChunk(
                        parent_item_id=new_item.id,
                        name=sanitize_html(dc.get("name", "")),
                        description=sanitize_html(dc.get("description")) if dc.get("description") else None,
                        format_type=dc.get("format_type"),
                        magnet_link=dc.get("magnet_link"),
